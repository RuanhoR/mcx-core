...
class Event {
  ...
  constructor(opt: EventOpt, loadExtend: (mcx: MCXFile<'event'>) => Event) {
    this.status.all = Object.fromEntries(
      Object.entries(opt.data).map(vl => {
        if (opt.tick) {
  ...
  private _bindEvent(
    ...
    if (eventName in this.status.on) {
      this.status.on[
        eventName as keyof (WorldAfterEvents | WorldBeforeEvents)
      ].subscribe(handler)
  ...
  private _remove_bind_event(
    ...
    if (eventName in this.status.on) {
      const name: keyof typeof this.status.on =
        eventName as keyof typeof this.status.on
  ...
  public unsubscribe(...arg: string[]): boolean {
    ...
    for (const item of arg) {
      ...
      if (!handler || typeof handler !== 'function') {
        success = false
        console.warn(
          "[mcx runtime]: can't find event handler for event: '" + item + "'",
        )
        continue
      }
      ...
      if (response.code !== 200) {
        success = false
        console.warn(
          `[mcx runtime]: bind event '${item}' error: ${response.msg} `,
        )
      }
    }
    return success
