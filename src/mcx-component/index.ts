...
export async function compileComponent(compiledCode: MCXCompileData, ctx: transformCtx) {
  ...
  const scriptRunResult = (await (new RunScript(compiledCode.File, "esm")).run(src, execESMMethod.transformCjs, (data, setData) => {
    if (setData && data.type == "CallExpression" && data.callee.type == "Identifier" && data.arguments.length == 1 && data.arguments[0]?.type == "CallExpression" && data.arguments[0].callee.type == "Identifier" && data.arguments[0].callee.name == "require") {
      ...
      if (arg && arg.type == "StringLiteral") {
        if (/^.+?\.(png|svg|jpg|jpeg|gif)$/.test(arg.value)) {
          const imageComponentRequire = t.memberExpression(t.callExpression(
  ...
  })) as Record<string, InstanceType<typeof lib[MCXstructureLocComponentType]> | undefined>
  // check have export
