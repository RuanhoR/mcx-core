...
class ImageComponent {
  ...
  constructor(public filePath: string, public imageType: string[]) {
    try {
      statSync(filePath);
      const ext = extname(filePath);
      if (!imageType.includes(ext.slice(1))) throw new Error("[image type]: file extname: " + ext + " !includes" + imageType)
    } catch (err) {
      throw new Error("[mcx image]: can't resolve image: " + filePath + " : \n  " + err)
    }
  }
}
