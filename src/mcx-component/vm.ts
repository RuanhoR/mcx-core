...
function transformESMToCJS(
  ...
  // 添加 import 转换
  const importDefines = transformImportIRtoRequire(compileData.BuildCache.import);
  defines.push(...importDefines);

  // 注入 plugin context
  if (pluginContext) {
    defines.push(...Object.entries(pluginContext).map(([key, value]): t.VariableDeclarator =>
  ...
  const exportsArr = compileData.BuildCache.export.map(i => {
    if (t.isExportAllDeclaration(i)) {
      const fileId = generateFileId();
      defines.push(t.variableDeclarator(
        t.identifier(fileId),
      ...
      return t.assignmentExpression(
        ...
        t.objectExpression([
          ...
          t.spreadElement(t.memberExpression(
            t.identifier("module"),
            t.identifier("exports")
          ))
        ])
      ...
      if (!i.declaration || t.isTSDeclareFunction(i.declaration)) return void 0;
      ...
      if (i.source && i.specifiers.length >= 1) {
        ...
        if (i.declaration) {
          if (t.isFunctionDeclaration(i.declaration) || t.isVariableDeclaration(i.declaration)) {
            if (t.isVariableDeclaration(i.declaration)) {
              ...
              return assignExprs.length === 1 ? assignExprs[0] : t.sequenceExpression(assignExprs);
            } else {
              const functionId = i.declaration.id || t.identifier(generateFileId());
 ...
 */
function transformImportIRtoRequire(importIR: ImportList[]): t.VariableDeclarator[] {
  let define: t.VariableDeclarator[] = [
    t.variableDeclarator(t.identifier("__import_default"), t.functionExpression(null, [t.identifier("obj")], t.blockStatement([
      t.returnStatement(
      ...
      if (!imported.isAll && imported.import) {
        if (imported.import == "default") {
          vl = t.callExpression(t.identifier("__import_default"), [
            t.callExpression(t.identifier("require"), [t.stringLiteral(data.source)])
          ]);
        } else {
        ...
        vl = t.callExpression(t.identifier("require"), [t.stringLiteral(data.source)]);
      }
      define.push(t.variableDeclarator(
        t.identifier(imported.as),
        vl
      ));
    }
  }
...
export class RunScript {
   ...
   */
  public async run(code: string, esmExecMethod: execESMMethod = execESMMethod.transformCjs): Promise<unknown> {
    if (this.module === 'esm') {
      if (esmExecMethod == execESMMethod.importESM) {
        ...
        if (this.pluginContext) {
          ...
          const contextDefines = Object.entries(this.pluginContext).map(([key, value]): t.VariableDeclarator =>
            t.variableDeclarator(
              t.identifier(key),
              typeof value == "string" ? t.stringLiteral(value) :
                typeof value == "boolean" ? t.booleanLiteral(value) :
          ...
          const contextDeclaration = t.variableDeclaration("var", contextDefines);
        ...
        const compiledCode = transformESMToCJS(code, this.pluginContext);
        ...
        if (typeof vm.SourceTextModule !== "function") {
          ...
          await script.link(async (specifier) => {
            return new vm.SourceTextModule(specifier, { context: this._context })
          })
  ...
  private getContext(pluginContext?: Record<string, unknown>): vm.Context {
    ...
    Object.assign(context, {
      exports,
      module,
      require: Module.createRequire ? Module.createRequire(this.filePath) : require,
      global: context
    })
